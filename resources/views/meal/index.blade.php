                    ...
                    <div class="card-body text-right ">
                        <ul class="list-group ">
                            ...
                            <a href="{{ route('meal.create') }}" class="list-group-item list-group-item-action">إضافة
                                وجبة</a>
                            <a href="{{route('admin')}}" class="list-group-item list-group-item-action">طلبات المستخدمين</a>
                              ...
                              @if (count($meals) > 0)
                                @foreach ($meals as $key => $row)
                                    ...
                                    <tr>
                                        ...
                                        <td><img src="{{asset('storage/'.$row->image) }}" width="80"></td>
                                        <td>{{$row->name  }}</td>
                                        ...
                                        <td style="width: 15%">
                                            <form action="{{ route('meal.delete',$row->id) }}" method="POST">
                                               @csrf
                                               @method('DELETE')
